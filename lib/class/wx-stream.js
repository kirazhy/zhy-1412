...
export default class WxStreamManager {
    ...
    constructor(url, {
        ...
        onData = (() => {}), onSuccess = (() => {}), onError = (() => {}), onComplete = (() => {})
    } = {}) {
        this.#url = url;
        this.#header = header;
    ...
    connect(data) {
        // 将输入data也返回
        const onChunkReceived = response => this.#onData(arrayBufferDecoder(response.data), data);
        this.#streamTask = wx.request({
            ...
            success: res => {
                ...
                console.log('Stream success', res);
            },
            // 失败-onError回调/打印日志
            fail: err => {
                ...
                if (err?.errMsg === 'request:fail abort') {
                    console.error('Stream 主动关闭');
                } else {
                    if (this.#errorToast) uni.showToast({title: this.#errorToast});
